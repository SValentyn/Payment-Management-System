    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws SQLException {
        ...
        if (method.equalsIgnoreCase(HTTPMethod.GET.name()) || method.equalsIgnoreCase(HTTPMethod.POST.name())) {
            ...
            if (!validation(request, currentUser, accountIdParam, cardIdParam)) {
                if (currentUser != null)
                    logging(currentUser.getUserId(), "ERROR: Unsuccessful attempt to detach card");
                return pathRedirect;
            }
            ...
            if (status == 0) {
                logging(currentUser.getUserId(), "ERROR: Unsuccessful attempt to detach card [" + card.getNumber() + "]");
                setSessionAttributes(request, ServerResponse.CARD_DETACHED_ERROR);
            } else {
                logging(currentUser.getUserId(), "DETACHED: Card [" + card.getNumber() + "]");
